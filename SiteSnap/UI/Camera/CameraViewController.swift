...
class CameraViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, CLLocationManagerDelegate, BackendConnectionDelegate, NewProjectViewControllerDelegate {
    ...
    var shouldNavigateToPhotos = true
...
//        if photoDatabaseShouldBeDeleted { // this will never execute
...
//    }
    ...
    func saveImage(image: UIImage!){
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                ...
                if !PhotoHandler.identifierAlreadyUploaded(localIdentifier: localId!) {
                    ...
                    self.photoObjects = PhotoHandler.fetchAllObjects(excludeUploaded: true)!
                    ...
                    alertController.addAction(UIAlertAction(title: NSLocalizedString("OK", comment: "Alert OK button"),
                                                            style: .default,
                                                            handler: {action in
                                                                let _ = PhotoHandler.removePhoto(localIdentifier: localId!)
                                                                if PhotoHandler.savePhotoInMyDatabase(localIdentifier: localId!, creationDate: createdDate!, latitude: self.lastLocation.coordinate.latitude, longitude: self.lastLocation.coordinate.longitude, isHidden: !isVisible){
                                                                    print("Photo added in core data")
                                                                }
                                                                PhotoHandler.setFileSize(localIdentifiers: [localId!])
    ...
    func saveDocumentImageToDatabase(fileName: String) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
            let now: Date = Date()
            if !PhotoHandler.identifierAlreadyUploaded(localIdentifier: fileName) {
                ...
                alertController.addAction(UIAlertAction(title: NSLocalizedString("OK", comment: "Alert OK button"),
                                                        style: .default,
                                                        handler: {action in
                                                            let _ = PhotoHandler.removePhoto(localIdentifier: fileName)
                                                            if PhotoHandler.savePhotoInMyDatabase(localIdentifier: fileName, creationDate: now, latitude: self.lastLocation.coordinate.latitude, longitude: self.lastLocation.coordinate.longitude, isHidden: true){
                                                                print("Photo added in core data")
                                                            }
                                                        }))
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "projectsListSegueFromCamera", let destination = segue.destination as? ProjectsListViewController {
    ...
    //MARK: - delete hidden and uploaded assets
    ...
    func assetsPicker(controller: AssetsPickerViewController, selected assets: [PHAsset]) {
        ...
        for phAsset in assets {
            ...
            if !PhotoHandler.identifierAlreadyUploaded(localIdentifier: phAsset.localIdentifier) {
                ...
                self.selectedFromGallery = false
                let alertController = UIAlertController(title: "SiteSnap", message: "This photo has already been uploaded before. Do you want to upload it again?.", preferredStyle: .alert)
                alertController.addAction(UIAlertAction(title: NSLocalizedString("OK", comment: "Alert OK button"),
                                                        style: .default,
                                                        handler: {action in
