...
class AddNewTagViewController: UIViewController, CircleCheckBoxDelegate, UITextFieldDelegate, BackendConnectionDelegate {
    ...
    @IBAction func onCancelTap(_ sender: Any) {
        dismiss(animated: true, completion: {
            self.delegate?.addNewTagwasDismissed()
        })
    }
    ...
    func treatErrorsApi(_ json: NSDictionary?) {
        //showToast(message: "\(json?["error"] ?? "null")", font: .systemFont(ofSize: 16))
        self.dismissProgressVC(completion: nil)
        if let json = json {
            self.treatErrorsAPI(json)
        }
    }
