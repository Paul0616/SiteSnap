...
//    }
    ...
    var projectIdToUploadToFromSharing: String?
    ...
    override func viewDidLoad() {
        ...
        var photos = PhotoHandler.fetchAllObjects(excludeUploaded: true)!
        for photo in photos {
...
//            if images.count == 1{
//                images.first?.state = .done
//                images.first?.date = Date()
//            }
            for image in images.filter({$0.state == .waiting}){
                image.state = .inProgress
                prepareUploadPhoto(localIdentifier: image.localIdentifier)
    ...
    @IBAction func onClickRemove(_ sender: UIButton) {
        ...
        let identifier = list[sender.tag].localIdentifier
        if PhotoHandler.removePhoto(localIdentifier: identifier){
            images.removeAll(where: {$0.localIdentifier == identifier})
            uploadedImagesTableConstraints()
            tableView2.reloadData()
        }
    }
    
    @IBAction func onClickRemoveAllHistory(_ sender: Any) {
        //
        let alertController = UIAlertController(title: "CLEAR UPLOAD HISTOTY", message: "Are you sure you want to clear all photo upload history on this device?\n\nThis action will not remove any photos that have already been uploaded to SiteSnap.", preferredStyle: .alert)
        alertController.addAction(UIAlertAction(title: NSLocalizedString("CLEAR", comment: "Confirm clear history"),
                                                style: .default,
                                                handler: { action in
                                                    self.alertAlreadyDisplayed = false
                                                    let identifiers: [String]? = PhotoHandler.getUploadedPhotosForDelete()
                                                    if let identifiers = identifiers, !identifiers.isEmpty {
                                                        if PhotoHandler.photosDeleteBatch(identifiers: identifiers){
                                                            print("history was cleared")
                                                            self.images.removeAll(where: {$0.state == .done})
                                                            self.uploadedImagesTableConstraints()
    ...
    @IBAction func onClickRetry(_ sender: UIButton) {
        ...
        let localIdentifier = list[sender.tag].localIdentifier
        if list[sender.tag].state == .fail ||  list[sender.tag].state == .waiting {
            if sessionManager == nil {
                createSessionManager()
            }
...
//    }
    ...
    private func makeImageDone(localIdentifier: String){
        ...
        var lastProjectToUploadedFor: String?
        if list.count > 0 {
            list.first?.state = .done
            list.first?.date = Date()
            lastProjectToUploadedFor = list.first?.projectName
        }
...
//            }
//        }
        if PhotoHandler.updateSuccessfulyUploaded(localIdentifier: localIdentifier, succsessfuly: true, lastProjectToUploadedFor: lastProjectToUploadedFor) {
                            ...
                            if let data = String(data: response.data!, encoding: .utf8){
                                ...
                                if let image = image {
                                    if (Int(data) ?? -1) != 0 {
                                        ...
                                        image.state = .fail
                                        if PhotoHandler.updateSuccessfulyUploaded(localIdentifier: identifier, succsessfuly: false, failCode: Int16(image.failCode), lastProjectToUploadedFor: nil ) {
                                            print("was signaled fail in database")
                                        }
                                        tableView1.reloadData()
                                    } else {
...
//                cell.cellContainerView.addGestureRecognizer(tapGesture)
        ...
        case tableView2:
            ...
            var myDate = ""
            var myTime = ""
            if let date = uploadedImages[indexPath.row].date{
                let formatter = DateFormatter()
                formatter.dateFormat = "dd/MM/YY"
                myDate = formatter.string(from: date)
                formatter.dateFormat = "HH:mma"
                myTime = formatter.string(from: date)
            }
            cell.sizeAndDate.text = "\(converByteToHumanReadable(uploadedImages[indexPath.row].fileSize))\n\(myTime)     \(myDate)"
